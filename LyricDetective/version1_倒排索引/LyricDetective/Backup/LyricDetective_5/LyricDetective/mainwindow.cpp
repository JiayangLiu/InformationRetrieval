...
void MainWindow::on_btnLRCtoTXT_clicked()
{
    ...
    QMessageBox::about(NULL, "Success", "已成功将 " + name_output + "转化为txt文件并储存");
}
...
void MainWindow::on_btnInvertedIndex_clicked()
{
    ...
    for (int i=0; i<listFileName.size(); i++) {
        ...
        index->StatWords(file_ToBeInverted.data(), file_CurrentProccessed); // 将string转换为char*
    }
    index->SaveResultToTXT();
}
