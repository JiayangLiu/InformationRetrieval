...
void LRCtoTXT::printLyric()
{
    ...
    QString txt_FileName = "/home/jeremyliu/Desktop/LyricDetective/txt_generated/" + file_CurrentProccess + ".txt"; // 指定文件夹路径和文件名
    QFile file(txt_FileName);
    // qDebug()<<"......."<<file.open(QIODevice::ReadWrite | QIODevice::Text);
    // file.open(QIODevice::WriteOnly | QIODevice::Text);
    if(! file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        QMessageBox::warning(NULL, "Warning", "无法录入文件 " + file_CurrentProccess + ".txt");
        return;
    } else {
        QString content;
        // 录入内容
